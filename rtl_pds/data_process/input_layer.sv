...
module input_layer #(
    ...
    generate
        ...
        for (genvar i = 0; i < KERNEL_ROW - STEP_ROW; i++) begin : gen_tops
            ...
            assign re_line_ram[i] = pp_rd_active;
            ...
            sdp_ram #(.WIDTH(FIFO_WIDTH), .DEPTH(LB_DEPTH)) u_lb_ram (
                ...
                .re(re_line_ram[i]), .raddr(lb_raddr), .q(line_ram_out[i])
            );
        end
        ...
        for (genvar i = KERNEL_ROW - STEP_ROW; i < KERNEL_ROW; i++) begin : gen_bottoms
            ...
            sdp_ram #(.WIDTH(FIFO_WIDTH), .DEPTH(PP_DEPTH)) u_pp_ram (
                ...
                .re(re_line_ram[i]), .raddr(pp_rd_cnt), .q(line_ram_out[i])
            );
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if(re_line_ram[0]) begin
                if (lb_raddr == LB_DEPTH - 1) 
                    lb_raddr <= 0;
                else 
                    lb_raddr <= lb_raddr + 1;
            end
            if(we_line_ram[0]) begin
    ...
    logic [KERNEL_COL-1:0] re_pixel_ram;
    logic [KERNEL_ROW-1:0][KERNEL_COL-1:0][FIFO_WIDTH-1:0] pixel_ram_out;
    logic [KERNEL_ROW-1:0][KERNEL_COL-1:0][FIFO_WIDTH-1:0] pixel_out;
    ...
    generate
        for (idx_page = 0; idx_page < PE_PAGE_NUM; idx_page++) begin : gen_pages
            for(idx_pe = 0; idx_pe < PE_ROW_NUM; idx_pe++) begin : gen_pe_rows
                always_ff @(posedge clk) begin
                    if(clk_en && stream_valid) begin :if_out
