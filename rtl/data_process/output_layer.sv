...
module output_layer #(
    ...
    parameter int unsigned PE_OUT_WIDTH     = 20, // PE 输出位宽
    parameter int unsigned BIAS_WIDTH       = 32, // Bias 位宽
    parameter int unsigned ACC_WIDTH        = 32, // 累加器位宽
    parameter int unsigned SHIFT_KEY        = 0,  // 右移位数
    parameter int unsigned OUT_WIDTH        = 8,  // 最终输出位宽
    parameter int unsigned CYCLE_PERIOD_OUT = 4,
    parameter int unsigned CYCLE_PERIOD_IN  = 3,
    ...
    parameter string       BIAS_FILE        = "biases_layer1.mem"
)(
    input  logic clk,
    input  logic clk_en,
    input  logic rst_n,
    ...
    // 来自 PE Page 的部分和输入
    input  logic signed [PE_OUT_WIDTH-1:0] page_y_out [PE_PAGE_NUM-1:0][PE_COL_NUM-1:0],
    
    // 最终输出
    output logic [OUT_WIDTH-1:0] final_out [PE_COL_NUM-1:0],
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if(!rst_n) acc_clear_pipe <= '0;
            ...
            for(int k=1; k<PE_COL_NUM; k++) begin
                acc_clear_pipe[k] <= acc_clear_pipe[k-1]; // 后续列依次延迟
            end
        end
    end
    ...
    genvar c, i;
    generate
        for (c = 0; c < PE_COL_NUM; c++) begin : col_proc
            ...
            always_ff @(posedge clk) begin
                if (clk_en) begin
                    ...
                    for (int k = 1; k < CYCLE_PERIOD_OUT - 1; k++) begin
                        acc_shift_reg[c][k] <= acc_shift_reg[c][k-1];
                    end
            ...
            always_ff @(posedge clk) begin
                if (clk_en) begin
                    sum_with_bias[c] <= sum_stage3[c] + bias_val[c];
                end
            end
            ...
            localparam int DESKEW_DELAY = PE_COL_NUM - c - 1;
            
            // 原始 Stage 5 输出结果
            // 延迟链定义
            logic [OUT_WIDTH-1:0] out_delay_pipe [DESKEW_DELAY-1:0];
            
            always_ff @(posedge clk) begin
                if (clk_en) begin
                    if(WITH_RELU == 1) begin
                        if (shifted_val_round[c] < 0) begin
                            stage5_result[c] <= 0;
                        end else begin
                            if (shifted_val_round[c] >= (1<<OUT_WIDTH)-1) 
                        ...
                        // 正向极值
                        if (shifted_val_round[c] >= (1<<(OUT_WIDTH-1))-1) 
                            stage5_result[c] <= {1'b0, {(OUT_WIDTH-1){1'b1}}};
                        // 负向极值
                        else if (shifted_val_round[c] <= -(1<<(OUT_WIDTH-1)))
            ...
            if (DESKEW_DELAY == 0) begin
                ...
                always_ff @(posedge clk) begin
                    if (clk_en) begin
                        ...
                        for (int k = 1; k < DESKEW_DELAY; k++) begin
                            out_delay_pipe[k] <= out_delay_pipe[k-1];
                        end
                    end
                end
