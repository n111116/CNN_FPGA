...
module char_overlay #(
    ...
    parameter string FONT_FILE = "chars_16x16.mem"
)(
    input  wire logic               clk_video,
    input  wire logic               rst_n_video,
    input  wire logic               video_vs_in,
    input  wire logic               video_hs_in,
    input  wire logic               video_de_in,
    input  wire logic [23:0]        video_rgb_in,
    output      logic               video_vs_out,
    output      logic               video_hs_out,
    output      logic               video_de_out,
    output      logic [23:0]        video_rgb_out,

    input  wire logic               clk_pe,
    input  wire logic               rst_n_pe,
    input  wire logic [15:0]        x_min_in,
    input  wire logic [15:0]        y_min_in,
    input  wire logic               new_line_1,

    input  wire logic [6:0]         out_char,
    input  wire logic               out_valid,
    input  wire logic               frame_start_out
);
    ...
    logic        c_valid [0:MAX_CHARS-1];
    logic [6:0]  c_code  [0:MAX_CHARS-1];
    logic [15:0] c_x     [0:MAX_CHARS-1];
    logic [15:0] c_y     [0:MAX_CHARS-1];
    logic [4:0]  wr_ptr; 
    ...
    always_ff @(posedge clk_video or negedge rst_n_video) begin
        if (!rst_n_video) begin
            wr_ptr <= 0;
            for (int i=0; i<MAX_CHARS; i++) c_valid[i] <= 0;
        end else begin
            ...
            for (int i=0; i<MAX_CHARS; i++) begin
                // 【修复2】去掉 pixel_y - 1，防止欠载溢出导致第0行误判
                if (c_valid[i] && hs_falling && pixel_y == (c_y[i] + 15)) begin
                    c_valid[i] <= 1'b0;
                end
            end
        end
    ...
    always_ff @(posedge clk_video) begin
        s1_hit <= 0;
        for (int i=0; i<MAX_CHARS; i++) begin
            if (c_valid[i] && video_de_in) begin
                if (pixel_x >= c_x[i] && pixel_x < c_x[i] + 16 &&
                    pixel_y >= c_y[i] && pixel_y < c_y[i] + 16) begin
                    s1_hit  <= 1'b1;
                    s1_char <= c_code[i];
                    s1_col  <= pixel_x - c_x[i];
                    s1_row  <= pixel_y - c_y[i];
                end
            end
        end
    end
...
// =========================================================
    ...
    logic [23:0] rgb_delay [0:2]; // 【修复】把 RGB 缓存缩短一级
    ...
    always_ff @(posedge clk_video) begin
        if (char_pixel) 
            ...
            // 【修复】取 T-3 时刻的数据进寄存器，输出正好是绝对延迟 4 拍！严格与 DE 信号对齐。
