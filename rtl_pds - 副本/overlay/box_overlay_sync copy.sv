module box_overlay_sync #(
    ...
    // [新增] 子图原始宽高透传接口，供后级 buffer manager 决定是否放大
    output logic [15:0]        crop_w0       [0:MAX_BOX_NUM-1],
    output logic [15:0]        crop_h0       [0:MAX_BOX_NUM-1],
    ...
    logic [MAX_BOX_NUM-1 : 0] box_started; // [新增] 用于标记是否经历了子图的起始行
    logic [7:0]         box_cnt; 
        ...
        if (!rst_n) begin
            ...
            // 无fifo_valid时
                ...
                // 剩余子图相关参数
                    ...
                    box_started[box_cnt] <= 1'b0; // [新增] 新框载入，将是否起点的标志清零
            ...
            // [修改点] 将判定框过期的逻辑移出 else 分支，并加入 box_started 的检测
            for (idx = 0; idx < MAX_BOX_NUM; idx++) begin
                if (box_valid[idx] && hs_falling) begin
                    // 如果遇到了框的起始行，则标记为已开始完整子图写入
                    if (pixel_y == box_ymin[idx]) begin
                        box_started[idx] <= 1'b1;
                    end

                    // 超出box边界后，且必须经历了完整的起始行，才将box置为无效框
                    if (pixel_y - 1 == box_ymax[idx]) begin
                        if (box_started[idx]) begin
                            box_valid[idx]   <= 1'b0;
                            box_started[idx] <= 1'b0; // 顺便重置状态
                        end
                    end
                end
            end
    ...
    // =========================================================
    // F. DDA 映射与子图 FIFO 生成逻辑 (仅缩小模式)
    // =========================================================
    generate
        for (i_gen = 0; i_gen < MAX_BOX_NUM; i_gen++) begin : gen_crop_fifo
            
            // [新增] 将宽、高透传给下级模块
            assign crop_w0[i_gen] = box_w0[i_gen];
            assign crop_h0[i_gen] = box_h0[i_gen];
