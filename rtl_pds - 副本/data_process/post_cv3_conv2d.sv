...
module post_cv3_conv2d #(
    // 数据流参数
    parameter DATA_WIDTH       = 13, // Layer33 输出位宽
    parameter OUT_WIDTH        = 32, // 打包输出位宽
    parameter SHIFT_KEY        = 7,
    parameter PE_COL_NUM       = 4,
    parameter CYCLE_PERIOD_OUT = 4,
    parameter CHANNEL_OUT_NUM  = CYCLE_PERIOD_OUT * PE_COL_NUM,
    parameter LUT_FILE         = "sigmoid_lut_9bit_to_8bit_h.mem",
    
    // 图像参数
    parameter IMG_COL          = 32, // Layer33 的输出分辨率宽
    parameter IMG_ROW          = 32, // Layer33 的输出分辨率高
    
    // 后处理参数
    parameter CONV_POSITIVE    = 1,  // 1=Max, 0=Min
    parameter CONF_WIDTH       = 8,  // 置信度位宽
    parameter CONF_THRESH      = 128 // 阈值 (0-255)
)(
    input  logic                    clk,
    input  logic                    rst_n,
    ...
    // [修改]：改为压缩数组
    input  logic [PE_COL_NUM-1:0][DATA_WIDTH-1:0]   data_in,
    ...
    // =============================================================
    // [修改]：去除了 mark_debug
    logic signed [DATA_WIDTH-1:0] raw_extrema;
    ...
    logic                  extrema_valid;
    // [修改]：改为压缩数组
    logic signed [PE_COL_NUM-1:0][DATA_WIDTH-1:0]   data_in_signed;
    ...
    // =============================================================
    // [修改]：去除了 mark_debug
    logic [CONF_WIDTH-1:0] confidence;
    logic                  conf_valid;
