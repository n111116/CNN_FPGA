...
vlog -work rtl_work -sv \
     +incdir+$INC_PATH \
     +define+DATA_PATH="$MEM_DATA_PATH" \
     [file join $ROOT_DIR sim_pds tb_lprnet_post_process.sv]
...
vsim -t 1ns -L rtl_work -L work +SEED=$rnd_seed -voptargs="+acc" tb_lprnet_post_process

# 7. 运行
do wave_lprnet_post_process.do
run -all
