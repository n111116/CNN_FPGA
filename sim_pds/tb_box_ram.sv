...
module tb_box_ram();
    ...
    // [全新架构] 内部 BRAM 读写与握手总线 (Downscaler -> Upscaler)
    logic        box_locked        [0:MAX_BOX_NUM-1];
    logic [15:0] box_x_min         [0:MAX_BOX_NUM-1];
    logic [15:0] box_y_min         [0:MAX_BOX_NUM-1];
    logic [15:0] bram_w            [0:MAX_BOX_NUM-1];
    logic [15:0] bram_h            [0:MAX_BOX_NUM-1];
    
    logic [11:0] bram_rd_addr      [0:MAX_BOX_NUM-1]; // 12位地址
    logic [23:0] bram_rd_data      [0:MAX_BOX_NUM-1];
    logic        release_toggle_pe [0:MAX_BOX_NUM-1];
    ...
    box_capture_downscaler #(
        ...
        .release_toggle_pe (release_toggle_pe)
    );
    ...
    begin
        ...
        for (int y = 0; y < IMG_HEIGHT; y++) begin
            if (frame_no == 0) begin
                ...
                if (y == 66) fork send_box_data(2, 4, 12, 99, 30, 10, 30, 10); join_none 
                if (y == 67) fork send_box_data(3, 5, 12, 99, 25, 10, 25, 10); join_none 
            end
